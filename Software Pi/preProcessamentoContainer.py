    ...
    def _proximaEtapa(self):
        if(not(self.exame['processamento'] is "S")):
            if(self._validate()):
                ...
                popup.geometry('{}x{}+{}+{}'.format(300, 60, 500, 300))
                ...
                etapaLabel = Label(popup, textvariable=etapaVar)
                etapaLabel.pack(side="bottom")
                popup.pack_slaves()
                ...
                quadrosIntro = int(self.exame['t2']) - int(self.exame['t1']) + 1
                quadrosProc = int(self.exame['t6']) - int(self.exame['t5']) + 1
                progress_step = float(100.0 / (quadrosProc + quadrosIntro))

                if (self.cap.isOpened()):

                    # Processa a média dos quadros iniciais
                    etapaVar.set("Definindo padrões...")

                    for x in range(quadrosIntro):
                        print("Processando: " + str(x + 1) + "/" + str(quadrosIntro))
                        progress += progress_step
                        progressVar.set(progress)
                        textVar.set(str(int(progress)) + "%")
                        popup.update()
                        self.cap.set(1, (int(self.exame['t1']) + x))
                        ret, frame = self.cap.read()

                        if ret:
                            if (x == 0):
                                area, circularidade, posHor, posVer, ang, frame = util.findIris(frame, 0, 0)
                            else:
                                area, circularidade, posHor, posVer, ang, frame = util.findIris(frame, 0, 0)
                            if (posHor != 0): introX.append(posHor)
                            if (posVer != 0): introY.append(posVer)
                            if (area != 0): introArea.append(area)
                            if (area != 0): introAngulo.append(ang)

                    self.exame['mediaX'] = np.median(introX)
                    self.exame['mediaY'] = np.median(introY)
                    self.exame['mediaArea'] = np.median(introArea)
                    self.exame['mediaAngulo'] = np.median(introAngulo)
                    self.exame['desvioX'] = np.std(introX)
                    self.exame['desvioY'] = np.std(introY)
                    self.exame['desvioArea'] = np.std(introArea)
                    self.exame['desvioAngulo'] = np.std(introAngulo)

                    etapaVar.set("Reconhecendo pupila...")
                    # Processa área do corte
                    for x in range(quadrosProc):
                        print("Processando: " + str(x + 1) + "/" + str(quadrosProc))
                        progress += progress_step
                        progressVar.set(progress)
                        textVar.set(str(int(progress)) + "%")
                        popup.update()

                        if ((x >= (int(self.eInicioCorte.get()) - int(self.exame['t5']))) and (
                                x <= (int(self.eFimCorte.get()) - int(self.exame['t5'])))):

                            self.cap.set(1, (int(self.exame['t5']) + x))
                            ret, frame = self.cap.read()

                            if ret:
                                if (x == (int(self.eInicioCorte.get()) - int(self.exame['t5']))):
                                    area, circularidade, posHor, posVer, ang, frame = util.findIris(frame, 0, 0)
                                else:
                                    area, circularidade, posHor, posVer, ang, frame = util.findIris(frame, 0, 0, 5)
                                cv2.imwrite(self.dir + "/frames/" + str(x + int(self.exame['t5'])) + ".png", frame)

                        else:
                            area = self.exame['mediaArea']
                            circularidade = 1
                            posVer = int(self.exame['mediaY'])
                            posHor = int(self.exame['mediaX'])
                            ang = 0
                    ...
                    etapaVar.set("Gerando arquivos...")
                    arq = open(self.dir + "/dados.exa", "w")
                    for i in range(quadrosProc):
                        arq.write(str(pos[i]))
                        arq.write(";")
                        arq.write(str(round(posArea[i], 2)))
                        arq.write(";")
